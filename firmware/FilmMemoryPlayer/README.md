...
- 調整幻燈片間隔、自動休眠時間與螢幕亮度。
- 可在設定頁顯示約略電量與電壓；S3 使用 GPIO11，XIAO C3 使用 D1/A1（GPIO3），兩者都需外接兩顆 100kΩ 分壓電阻。
- 長按旋鈕進入低功耗休眠，再按一下喚醒。
- 自動保存設定與最後選擇的表情。
...
| `SETTINGS` | 修改目前選項 | 切換下一個選項 | 設定幻燈片、自動休眠與亮度，並顯示約略電量 |
...
├─ BatteryMonitor.h/.cpp  # S3/C3 電池電壓與百分比估算
