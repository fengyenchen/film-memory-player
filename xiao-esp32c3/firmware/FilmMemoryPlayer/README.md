# Film Memory Player 韌體｜Seeed Studio XIAO ESP32-C3

這個資料夾是 Film Memory Player 的 **Seeed Studio XIAO ESP32-C3 完整韌體**。程式使用 128×128 SPI TFT、EC11 旋轉編碼器與板載 LittleFS，提供照片播放、時鐘、表情、Wi-Fi 相簿管理、設定與休眠功能。
...
| `SETTINGS` | 修改目前選項 | 切換下一個選項 | 設定幻燈片與自動休眠 |

幻燈片可選 `2`、`3.5`、`5`、`10` 秒；自動休眠可選 `OFF`、`1`、`3`、`5` 分鐘。
...
如果使用需要外接天線的版本，請確認 IPEX 天線已垂直扣緊。
...
├─ Config.h               # XIAO ESP32-C3 GPIO、尺寸與常數
...
├─ PowerManager.h/.cpp    # GPIO21 休眠喚醒
